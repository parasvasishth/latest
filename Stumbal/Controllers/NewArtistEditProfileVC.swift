//
//  NewArtistEditProfileVC.swift
//  Stumbal
//
//  Created by Dr.Mac on 04/01/22.
//

import UIKit
import Alamofire
import Kingfisher
class NewArtistEditProfileVC: UIViewController,UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UITableViewDataSource,UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UISearchBarDelegate,UITextViewDelegate {
    ...
    @IBOutlet weak var spotiLbl: UILabel!
...
//        linkTxtView.text = "Enter link"
//        linkTxtView.textColor = UIColor.white
//
        
        linkTxtView.placeholder = "Enter link"
      
//               placeholderLabel = UILabel()
//               placeholderLabel.text = "Enter link"
...
//        linkTxtView.addSubview(placeholderLabel)
//               placeholderLabel.frame.origin = CGPoint(x: 5, y: (linkTxtView.font?.pointSize)! / 2)
//               placeholderLabel.textColor = UIColor.white
//               placeholderLabel.isHidden = !linkTxtView.text.isEmpty
//        linkTxtView.delegate = self
...
//         if linkTxtView.text == "Enter link" {
//            linkTxtView.text = ""
//            linkTxtView.textColor = UIColor.white
//        }
//    }
...
//       }
    ...
    @IBAction func spotiBtn(_ sender: UIButton) {
        ...
        if linkTxtView.text != ""
        {
            if UserDefaults.standard.bool(forKey: "spoti") == true
            {
                ...
                if spotiString != ""
                {
                    ...
                    spotiLbl.textColor = UIColor.white
                }
                else
                {
                    spotiLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                    spotiImg.image = UIImage(named: "gspotify")
                }
            }
            else if UserDefaults.standard.bool(forKey: "insta") == true
            {
                instaString = linkTxtView.text!
            ...
            {
                twitterString = linkTxtView.text!
    ...
    @IBAction func twitter(_ sender: UIButton) {
        ...
        UserDefaults.standard.set(false, forKey: "band")
        UserDefaults.standard.set(false, forKey: "cloud")
        linkTxtView.text = twitterString
        hideView.isHidden = false
        linkView.isHidden = false
    }
            ...
            if let data = response.data
            {
                ...
                
                {
                    ...
                    if  json["result"] as! String == "success"
                    {
                        ...
                        let alert = UIAlertController(title: "", message: "You have successfully updated profile…", preferredStyle: .alert)
                        self.present(alert, animated: true, completion: nil)
        ...
        if let data = response.data
        {
            ...
            else
            {
...

                {
                    ...
                    self.spotiString = json["spotify"] as! String
                    self.instaString = json["instagram"] as! String
                    self.twitterString = json["twitter"] as! String
                    self.bandString = json["band_camp"] as! String
                    self.cloudString = json["sound_cloud"] as! String
                    
                    if self.spotiString != ""
                    {
                        self.spotiImg.image = UIImage(named: "spotifyp")
                        self.spotiLbl.textColor = UIColor.white
                    }
                    else
                    {
                        self.spotiLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                        self.spotiImg.image = UIImage(named: "gspotify")
                    }
                    
                    if self.instaString != ""
                    {
                        self.instaImg.image = UIImage(named: "pinsta")
                        self.instaLbl.textColor = UIColor.white
                    }
                    else
                    {
                        self.instaLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                        self.instaImg.image = UIImage(named: "ginsta")
                    }
                    
                    if self.twitterString != ""
                    {
                        self.twiImg.image = UIImage(named: "twitterp")
                        self.twiLbl.textColor = UIColor.white
                    }
                    else
                    {
                        self.twiLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                        self.twiImg.image = UIImage(named: "twitterg")
                    }
                    
                    if self.bandString != ""
                    {
                        self.pentaImg.image = UIImage(named: "pentap")
                        self.pentaLbl.textColor = UIColor.white
                    }
                    else
                    {
                        self.pentaLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                        self.pentaImg.image = UIImage(named: "pentag")
                    }
                    
                    if self.cloudString != ""
                    {
                        self.cloudImg.image = UIImage(named: "cloudp")
                        self.cloudLbl.textColor = UIColor.white
                    }
                    else
                    {
                        self.cloudLbl.textColor = #colorLiteral(red: 0.2352941176, green: 0.2392156863, blue: 0.2352941176, alpha: 1)
                        self.cloudImg.image = UIImage(named: "cloudg")
                    }
                    
                self.artistNameField.text = json["name"] as! String
                    self.bioTxtView.text = json["bio"] as! String
                    let b:String = json["bio"] as! String
                    self.countLbl.text = String(b.count) + "/100"

                    self.categoryArray = json["category_detail"] as! NSArray
                    self.other = json["other_user"] as! NSArray
    
                    if self.categoryArray.count != 0
                    {
                        self.servicenameArr = self.categoryArray as! [[String : String]]
                        self.artistArray = NSMutableArray(array:self.servicenameArr)
                    
                        self.catTblHeight.constant = 180
                        self.catTblView.isHidden = false

                       //self.tblHeight.constant = 400
                        self.catTblView.reloadData()
                    }
                    else
                    {
                        //self.categoryLblHeight.constant = 0
                        self.catTblHeight.constant = 0
                        self.catTblView.isHidden = true
                    }
                    
                    if self.other.count != 0
                    {
                        self.servicenameArr1 = self.other as! [[String : String]]
                        self.artistlistArray = NSMutableArray(array:self.servicenameArr1)
                        UserDefaults.standard.set(true, forKey: "addother")
                        UserDefaults.standard.set(false, forKey: "searchother")
                        self.artistTblView.isHidden = false
                        
                        if self.artistlistArray.count == 1
                        {
                            self.artistTblHeight.constant = 100
                        }
                        else
                        {
                            self.artistTblHeight.constant = 185
                      ...
                      //  self.listViewHeight.constant = 185

...
import Alamofire
import Kingfisher
class BlockListVC: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate {
    ...
    var blockArr:NSMutableArray = NSMutableArray()
    @IBOutlet weak var loadingView: UIView!
    override func viewDidLoad() {
        super.viewDidLoad()
        self.loadingView.isHidden = false
        blockListTblView.dataSource = self
        blockListTblView.delegate = self
    ...
    @IBAction func userProfile(_ sender: UIButton) {
        ...
        UserDefaults.standard.set((blockArr.object(at: tagVal) as AnyObject).value(forKey: "block_user_id")as! String, forKey: "friend_rid")
    ...
    //MARK: fetch_block_friendlist ;
    func fetch_block_friendlist()
    {
...
//        hud.label.text = "Loading...."
     ...
     //   https://stumbal.com/process.php?action=get_last_msg
       // https://stumbal.com/process.php?action=fetch_friend_list
        ...
        Alamofire.request("https://stumbal.com/process.php?action=fetch_block_friendlist", method: .post, parameters: ["user_id":uID,"search":searchBar.text!], encoding:  URLEncoding.httpBody).responseJSON { [self] response in
            if let data = response.data {
                ...
                else
                {
                    do  {
                        MBProgressHUD.hide(for: self.view, animated: true);
                        self.blockArr = NSMutableArray()
                        self.blockArr =  try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.mutableContainers) as! NSArray as! NSMutableArray
                        print("1111",self.blockArr)
                        
                        
                        if self.blockArr.count != 0 {
                            self.searchBar.isHidden = false
                            self.statusLbl.isHidden = true
                            self.searchImg.isHidden = false
                            //self.statusLbl.text = "You have no friend"
                            self.blockListTblView.isHidden = false
                            ...
                            self.loadingView.isHidden = true
                            self.blockListTblView.isHidden = true
...
func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let cell =  blockListTblView.dequeueReusableCell(withIdentifier: "NewFriendOnEventTableViewCell", for: indexPath) as! NewFriendOnEventTableViewCell
    ...
    cell.profileView.clipsToBounds = true
        ...
        cell.nameLbl.text = (blockArr.object(at: indexPath.row) as AnyObject).value(forKey: "name")as! String


        let pimg:String = (blockArr.object(at: indexPath.row) as AnyObject).value(forKey: "user_image")as! String
